...
const searchNearByRestaurants = async (latitude, longitude, radius) => {
    const googleResponse = await axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=${radius}&type=restaurant&opennow=true&key=${GOOGLE_MAPS_API_KEY}`);
...
router.get('/restaurants', async (req, res) => {
    ...
    try {
        const places = await searchNearByRestaurants(latitude, longitude, 24140);
        const filteredPlaces = [];

        for (let basicDetails of places) {

            let yelpDetails;
            try {
                //UNCOMMENT BELOW TO GET YELP DETAILS WORKING
                yelpDetails = await yelpLogic.getYelpRestaurantDetails(basicDetails.lat, basicDetails.lng, basicDetails.business_name);
...
module.exports = { router, searchNearByRestaurants };
