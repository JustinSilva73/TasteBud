...
router.post('/pos', async (req, res) => {
    ...
    console.log('Starting to get restaurant from position')
    try {
        ...
        if (yelpRes) {
            console.log('recent restaurant', yelpRes.yelpID, yelpRes.categories, yelp.address)
            const queryRecentRes = `INSERT INTO tastebud.RecentRestaurants (userID, restaurantName, yelpID, restaurantAddress)
                                    VALUES (?, ?, ?, ?)`
            db.query(queryRecentRes, [user_id, yelpRes.name, yelpRes.yelpID, address], (err, results) => {
                if (err) {
